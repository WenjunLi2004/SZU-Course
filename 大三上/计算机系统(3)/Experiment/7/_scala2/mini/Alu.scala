...
class AluSimple(val width: Int) extends Alu {
  ...
  io.out := MuxLookup(io.alu_op, io.B)(
    Seq(
      ...
      // ABS_SUB 逻辑
      ALU_ABS_SUB -> Mux((io.A.asSInt - io.B.asSInt) < 0.S, -(io.A.asSInt - io.B.asSInt), (io.A.asSInt - io.B.asSInt)).asUInt
    )
  )
...
class AluArea(val width: Int) extends Alu {
  ...
  // ABS_SUB 逻辑
  val abs_sub = Mux((io.A.asSInt - io.B.asSInt) < 0.S, -(io.A.asSInt - io.B.asSInt), (io.A.asSInt - io.B.asSInt)).asUInt
