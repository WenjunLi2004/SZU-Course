...
object Control {
  ...
  // COMB指令
    COMB  -> List(PC_4  , A_RS1,  B_RS2, IMM_J, ALU_COMB , BR_XXX, N, ST_XXX, LD_XXX, WB_ALU, N, CSR.N, N),

  // ABS_SUB 的控制信号
  // 重点检查: WB_ALU (写回ALU结果), Y (写使能开启)
    ABS_SUB -> List(PC_4, A_RS1, B_RS2, IMM_X, ALU_ABS_SUB, BR_XXX, N, ST_XXX, LD_XXX, WB_ALU, Y, CSR.N, N))
  // format: on
}
