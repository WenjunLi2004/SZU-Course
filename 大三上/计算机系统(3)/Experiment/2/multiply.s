...
.data
CTRL_REG:   .word 0x10000                  # 控制寄存器地址
DATA_REG:   .word 0x10008                  # 数据寄存器地址
...
############################################################
printText:
	daddi $sp, $sp, -4            # 为返回地址分配栈空间
	sw $ra, 0($sp)                # 保存返回地址

	sw $a0, 0($a1)                # 将字符串地址写入数据寄存器
	daddi $t0, $zero, 4           # 控制码4代表打印字符串
	sw $t0, 0($a2)                # 向控制寄存器写入4，触发输出
...
main:
	...
	lw $a1, DATA_REG($zero)       # 将DATA寄存器地址加载到$a1
	lw $a2, CTRL_REG($zero)       # 将CONTROL寄存器地址加载到$a2
