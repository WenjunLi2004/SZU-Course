...
{
    cleanData();
    // 圆盘放置在 z=0 平面，采用三角扇拓扑（中心点 + 环周点）
    int num_samples = num_division;
    float step = 2 * M_PI / num_samples; // 周向采样角步长

    // 中心顶点（几何 + 颜色 + UV）。UV中心为(0.5,0.5)，便于将纹理中心对齐原点
    vertex_positions.push_back(glm::vec3(0.0, 0.0, 0.0));
    vertex_colors.push_back(glm::vec3(1.0, 1.0, 1.0));
    vertex_textures.push_back(glm::vec2(0.5f, 0.5f));

    // 环周顶点：按半径r在平面上离散采样；UV将平面坐标归一化到[0,1]
    for (int i = 0; i < num_samples; i++) {
        float a = i * step;
    ...
    // 组装三角面片：中心点与相邻两个环周点构成一个三角形
    for (int i = 0; i < num_samples; i++) {
        int a = 0;
        int b = i + 1;
        int c = (i + 1) % num_samples + 1;
        faces.push_back(vec3i(a, b, c));
    }

    // 索引一致：每个三角面的坐标/颜色/法线/纹理索引一致，纹理坐标与顶点一一对应
    normal_index = faces;
    color_index = faces;
    texture_index = faces;
...
{
    cleanData();
    // 圆锥沿Z轴生成：底面在 z=-height，顶点在 z=+height（在应用侧通过旋转使其“朝上”）
    int num_samples = num_division;
    float step = 2 * M_PI / num_samples; // 周向采样角步长

    // 生成底缘顶点（仅侧面，不包含底面盘），颜色用单位化的XY方向以便观察
    float zBase = -height;
    for (int i = 0; i < num_samples; i++) {
        float a = i * step;
        float x = radius * cos(a);
        float y = radius * sin(a);
        vertex_positions.push_back(glm::vec3(x, y, zBase));
        vertex_colors.push_back(normalize(glm::vec3(x, y, 0.0f)));
    }

    // 顶点（apex）
    int apexIndex = vertex_positions.size();
    vertex_positions.push_back(glm::vec3(0.0f, 0.0f, height));
    vertex_colors.push_back(glm::vec3(1.0, 1.0, 1.0));

    // 侧面三角：每段由两个相邻底缘点与顶点构成
    // UV 展开：底缘 v=0，u=周向归一化；顶点固定(0.5,1.0)匹配纹理三角形顶点，避免接缝错位
    for (int i = 0; i < num_samples; i++) {
        faces.push_back(vec3i(i, (i + 1) % num_samples, apexIndex));

        float u0 = 1.0f * i / num_samples;
        float u1 = 1.0f * (i + 1) / num_samples;
        vertex_textures.push_back(glm::vec2(u0, 0.0f));
        vertex_textures.push_back(glm::vec2(u1, 0.0f));
        vertex_textures.push_back(glm::vec2(0.5f, 1.0f));
