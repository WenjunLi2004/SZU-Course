...
void MeshPainter::bindObjectAndData(TriMesh *mesh, openGLObject &object, const std::string &texture_image, const std::string &vshader, const std::string &fshader){
    ...
    // 将纹理坐标传入着色器：
    // 1) 顶点着色器中声明了 `in vec2 vTexture`，用于接收每个顶点的UV
    // 2) 我们在同一个VBO中依次写入了 position、color、normal、texture 四段数据
    //    因此纹理数据的起始偏移量为 (points+colors+normals) * sizeof(vec3)
    // 3) 每个纹理坐标由两个浮点数组成（u,v），不需要归一化和步进（紧密排列）
