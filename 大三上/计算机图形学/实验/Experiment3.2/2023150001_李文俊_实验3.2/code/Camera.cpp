...
glm::mat4 Camera::lookAt(const glm::vec4& eye, const glm::vec4& at, const glm::vec4& up)
{
	...
	// 计算相机坐标系的三个轴向量	// 列主序的标准 LookAt 视图矩阵实现
	glm::vec3 n = glm::normalize(glm::vec3(eye - at)); // 视线反向（从at指向eye）
	glm::vec3 u = glm::normalize(glm::cross(glm::vec3(up), n)); // 右向量
	glm::vec3 v = glm::cross(n, u); // 上向量

	glm::mat4 c = glm::mat4(1.0f);
	// 将基向量按列填充
	c[0][0] = u.x; c[1][0] = u.y; c[2][0] = u.z; c[3][0] = -glm::dot(u, glm::vec3(eye));
	c[0][1] = v.x; c[1][1] = v.y; c[2][1] = v.z; c[3][1] = -glm::dot(v, glm::vec3(eye));
	c[0][2] = n.x; c[1][2] = n.y; c[2][2] = n.z; c[3][2] = -glm::dot(n, glm::vec3(eye));
...
glm::mat4 Camera::perspective(const GLfloat fovy, const GLfloat aspect,
	const GLfloat zNear, const GLfloat zFar)
{
	GLfloat top = tan(fovy * M_PI / 180.0f / 2.0f) * zNear;
