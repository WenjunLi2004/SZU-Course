...
std::vector<vec3> generateFractalTree()
{
	...
	{
		if (depth <= 0 || branchLength < 0.008f)
			return;
...
void init()
{
	...
	*/
	...
	glGenVertexArrays(1, &vao[2]); // 生成新的VAO
	...
	cLocation = glGetAttribLocation(program, "vColor");
	glEnableVertexAttribArray(cLocation);
	glVertexAttribPointer(cLocation, 3, GL_FLOAT, GL_FALSE, sizeof(glm::vec3), BUFFER_OFFSET(0));

	/*
	* 初始化椭圆的数据
	*/

	glGenVertexArrays(1, &vao[3]); // 生成新的VAO
...
void key_callback(GLFWwindow *window, int key, int scancode, int action, int mods)
{
	if (action == GLFW_PRESS)
	{
		...
		case GLFW_KEY_1:
			currentPattern = 0;
			std::cout << "Switch to ellipse rotation pattern" << std::endl;
			break;
...
int main(int argc, char **argv)
{
	...
	// 配置窗口属性
	GLFWwindow *window = glfwCreateWindow(512, 512, "2023150001_liwenjun_Experiment1", NULL, NULL);
	if (window == NULL)
	{
