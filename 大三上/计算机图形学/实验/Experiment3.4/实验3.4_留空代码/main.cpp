...
// Task3: 光照分量开关（1 开启 / 0 关闭）
int ambientOpenFlag = 1;
int diffuseOpenFlag = 1;
int specularOpenFlag = 1;
...
void bindObjectAndData(TriMesh *mesh, openGLObject &object, const std::string &vshader, const std::string &fshader)
{
    ...
    glBufferSubData(GL_ARRAY_BUFFER,
                    (mesh->getPoints().size() + mesh->getColors().size()) * sizeof(glm::vec3),
...
void display()
{
	...
	// Task3: 设置环境/漫反射/镜面反射开关到着色器
	glUniform1i(mesh_object.ambientOpen, ambientOpenFlag);
	glUniform1i(mesh_object.DiffuseOpen, diffuseOpenFlag);
	glUniform1i(mesh_object.SpecularOpen, specularOpenFlag);
...
void mainWindow_key_callback(GLFWwindow *window, int key, int scancode, int action, int mode)
{
	...
	{
		ambientOpenFlag = 1 - ambientOpenFlag;
	}
	else if (key == GLFW_KEY_F6 && action == GLFW_PRESS)
	{
		diffuseOpenFlag = 1 - diffuseOpenFlag;
	}
	else if (key == GLFW_KEY_F7 && action == GLFW_PRESS)
	{
		specularOpenFlag = 1 - specularOpenFlag;
	}
	else
	{
...
int main(int argc, char **argv)
{
	...
	GLFWwindow *mainwindow = glfwCreateWindow(600, 600, "2023150001_李文俊_实验3.4", NULL, NULL);
	if (mainwindow == NULL)
	{
