...
void TriMesh::readObj(const std::string& filename)
{
	...
	while (std::getline(fin, line))
	{
		...
		if (type == "v") {
			...
			std::string v_str[3];
			sin >> v_str[0] >> v_str[1] >> v_str[2];
			
			int v_idx[3], vt_idx[3], vn_idx[3];
			
			for (int i = 0; i < 3; i++) {
				// sscanf is safer to parse formatted string
				// Indices in OBJ are 1-based, need to convert to 0-based
				sscanf(v_str[i].c_str(), "%d/%d/%d", &v_idx[i], &vt_idx[i], &vn_idx[i]);
				v_idx[i]--;
				vt_idx[i]--;
				vn_idx[i]--;
			}
			...
			color_index.push_back(vec3i(vn_idx[0], vn_idx[1], vn_idx[2]));
		}
	}
