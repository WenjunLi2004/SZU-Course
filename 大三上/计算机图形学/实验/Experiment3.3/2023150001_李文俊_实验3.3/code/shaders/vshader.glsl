...
void main() 
{
    ...
    vec3 posView = (ModelView * vec4(vPosition, 1.0) ).xyz;
    vec3 lPosView = (view * vec4(light.position, 1.0) ).xyz;
    vec3 normView = (ModelView * vec4(vNormal, 0.0)).xyz;

    // @TODO: Task2 计算四个归一化的向量 N,V,L,R(或半角向量H)
    vec3 N = normalize(normView);
    vec3 V = normalize(-posView);
    vec3 L = normalize(lPosView - posView);
