...
def train_and_evaluate_models(inputfile="model.xls"):
    ...
    net_prob = net.predict_proba(x_test)[:, list(net.classes_).index(1.0)]
    net_cm = confusion_matrix(y_test, net_pred, labels=[0, 1])
    net_fpr, net_tpr, _ = roc_curve(y_test, net_prob, pos_label=1)
    net_auc = auc(net_fpr, net_tpr)
